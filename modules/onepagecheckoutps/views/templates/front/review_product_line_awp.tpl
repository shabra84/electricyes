...
<div class="row {if isset($productLast) and $productLast && (not isset($ignoreProductLast) or !$ignoreProductLast)}last_item{elseif isset($productFirst) and $productFirst}first_item{/if} {if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0}alternate_item{/if} cart_item address_{$product.id_address_delivery|intval}"
    ...
    <div class="col-md-{if $CONFIGS.OPC_SHOW_UNIT_PRICE}4{else}6{/if} col-xs-9 col-9 cart_description">
        ...
        {if $product.weight neq 0 and $CONFIGS.OPC_SHOW_WEIGHT}
            ...
            <span class="product_weight">
                <strong>{l s='Weight' mod='onepagecheckoutps'}&nbsp;:&nbsp;</strong>
    ...
    {if $CONFIGS.OPC_SHOW_UNIT_PRICE}
        ...
        <div class="col-md-3 col-xs-9 col-9 text-md-center text-xs-left text-sm-left">
            <span class="" id="product_price_7_34_0">
                <span class="price special-price">{if isset($mega)}{$mega.spricewt}{else}{$product.price}{/if}</span>
                <br>
    ...
    <div class="col-md-2 col-xs-9 col-9 text-md-center">
        <div class="input-group bootstrap-touchspin">
            ...
            <input
                ...
                data-down-url="{$product.down_quantity_url}&special_instructions={$product.instructions_valid}&special_instructions_id={$product.instructions_id}{if isset($mega)}&id_megacart={$mega.id_megacart}{/if}"
                data-up-url="{$product.up_quantity_url}&special_instructions={$product.instructions_valid}&special_instructions_id={$product.instructions_id}{if isset($mega)}&id_megacart={$mega.id_megacart}{/if}"
                data-update-url="{$product.update_quantity_url}&special_instructions={$product.instructions_valid}&special_instructions_id={$product.instructions_id}{if isset($mega)}&id_megacart={$mega.id_megacart}{/if}"
                data-product-id="{$product.id_product}"
                awp-data-product-attribute-id="{$product.id_product_attribute}"
                {if isset($mega)}data-mega-id="{$mega.id_megacart}"{/if}
                type="text"
                value="{if isset($mega)}{$mega.quantity}{else}{$product.quantity}{/if}"
            ...
            <a
                ...
                rel                         = "nofollow"
                href                        = "{$product.remove_from_cart_url}&special_instructions={$product.instructions_valid}{if isset($mega)}&id_megacart={$mega.id_megacart}{/if}"
    ...
    <div class="col-md-2 col-xs-9 col-9 text-md-right text-xs-left text-sm-left">
        <span class="product-price pull-right">{if isset($mega)}{$mega.stotalwt}{else}{$product.total}{/if}</span>
    </div>
</div>