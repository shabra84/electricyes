...
{block name='step_review'}
    ...
    {if $CONFIGS.OPC_SHOW_REMAINING_FREE_SHIPPING}
        {if $cart.subtotals.shipping.amount > 0 && !isset($virtualCart) && $free_ship}
            <div id="remaining_amount_free_shipping" class="alert alert-info text-center">
                ...
                <span id="free_shipping"><b>{$free_ship}</b></span>
            </div>
        {/if}
    {/if}
    ...
    <div id="order-detail-content" class="cart-detailed-totals">
        {foreach from=$cart.products|@sortby:'name' item=product}
            ...
            {if isset($product.productmega)}
                {foreach from=$product.productmega item=mega name=productMegas}
                    {if isset($attributewizardpro)}
                        {include file="./review_product_line_awp.tpl" CONFIGS=$CONFIGS productLast=$product@last productFirst=$product@first mega=$mega}
                    {else}
                        {include file="./review_product_line.tpl" CONFIGS=$CONFIGS productLast=$product@last productFirst=$product@first mega=$mega}
                    {/if}
                {/foreach}
            {else}
                {if isset($attributewizardpro)}
