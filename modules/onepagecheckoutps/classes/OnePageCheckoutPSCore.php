...
class OnePageCheckoutPSCore
{
    ...
    private function fillGlobalVars()
    {
        ...
        $this->module->globals->lang->type_control = array(
            'select'   => $this->module->l('List'),
            'textbox'  => $this->module->l('Textbox'),
            'textarea' => $this->module->l('Textarea'),
            'radio'    => $this->module->l('Radio button'),
            'checkbox' => $this->module->l('Checkbox')
        );
    }
    ...
    public function getContent()
    {
        if (!function_exists('curl_init')
            ...
            $this->module->errors[] = $this->module->l('CURL functions not available for registration module.');
        } else {
            $id_shop_group = 0;
            $id_shop       = 0;
            ...
            if (Tools::isSubmit('sent_register')) {
                if (Tools::getIsset('email') && Tools::getIsset('number_order')) {
                    ...
                    if (!empty($params['email']) && !empty($params['number_order'])) {
                        ...
                        $this->module->errors[] = 'Please enter the information marked as mandatory for registration module.';
                    }
                }
                ...
                if (Tools::getIsset('license_number') && !empty($params['license_number'])) {
                    ...
                    $this->module->errors[] = 'Please enter the license to do the validation of the module.';
                }
            }
        }
    }
    ...
    public function displayForm()
    {
        ...
        $this->context->controller->addCSS($this->module->path.'views/css/lib/bootstrap/pts/pts-bootstrap.css', 'all');
        $this->context->controller->addCSS($this->module->path.'views/css/lib/simple-switch/simple-switch.css', 'all');

        //back
        $this->context->controller->addJS($this->module->path.'views/js/admin/configure.js?v='.$this->module->version);
        $this->context->controller->addJS($this->module->path.'views/js/lib/pts/tools.js?v='.$this->module->version);

        $this->context->controller->addCSS($this->module->path.'views/css/admin/configure.css', 'all');
        $this->context->controller->addCSS($this->module->path.'views/css/lib/pts/tools.css', 'all');
        $this->context->controller->addCSS($this->module->path.'views/css/lib/pts/pts-menu.css', 'all');
        $this->context->controller->addCSS($this->module->path.'views/css/lib/font-awesome/font-awesome.css', 'all');
    ...
    private function saveConfigValue($option)
    {
        ...
        if (array_key_exists($config_var_name, $this->module->config_vars)) {
            ...
            if (!Configuration::updateValue($config_var_name, $config_var_value, $is_html)) {
                $this->module->errors[] = $this->module->l('An error occurred while trying update').': '.$option['label'];
            }
    ...
    public function saveContentCodeEditors()
    {
        ...
        if (is_writable($filepath)) {
            ...
            return array(
                ...
                'message' => $this->module->l('The code was successfully saved')
            );
        }

        return array(
            'message_code' => $this->CODE_ERROR,
            'message' => $this->module->l('Failed to save changes')
        );
    }
    ...
    public function readFile($module, $iso_code, $default = false)
    {
        ...
        $file = fopen($file_path, 'r') or exit($this->module->l('Unable to open file'));
    ...
    public function shareTranslation($iso_code)
    {
        ...
        if (file_exists($file_path)) {
            ...
            $send = Mail::Send(
                $id_lang,
                'test',
                $this->module->l('he shared a translation with you').' - PS:'._PS_VERSION_.' - MOD:'.$this->module->version,
                array(),
            ...
            if ($send) {
                return array(
                    'message_code' => $this->CODE_SUCCESS,
                    'message' => $this->module->l('Your translation has been sent, we will consider it for future upgrades of the module')
                );
            }
        }
        ...
        return array(
            'message_code' => $this->CODE_ERROR,
            'message' => $this->module->l('An error has occurred to attempt send the translation')
        );
    }
    ...
    public function saveTranslations($iso_code)
    {
        ...
        if (is_writable($file_path)) {
            ...
            if (!file_put_contents($file_path, $line)) {
                return array(
                    'message_code' => $this->CODE_ERROR,
                    'message' => $this->module->l('An error has occurred while attempting to save the translations')
                );
            }
            ...
            return array(
                'message_code' => $this->CODE_SUCCESS,
                'message' => $this->module->l('The translations have been successfully saved')
            );
        }
