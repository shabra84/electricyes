...
require_once (dirname(__FILE__).'/classes/TmSearchProvider.php');

class Tmsearch extends Module implements WidgetInterface
{
    ...
    private function getCategoriesList()
    {
        ...
        $this->generateCategoriesOption($category->getNestedCategories((int)Configuration::get('PS_HOME_CATEGORY')), true);
    ...
    public function hookHeader()
    {
        ...
        $this->context->controller->registerStylesheet('module-tmsearch', 'modules/'. $this->name .'/views/css/tmsearch.css');
        ...
        if (Configuration::get('PS_TMSEARCH_AJAX')) {
            ...
            $this->context->controller->registerJavascript('module-tmsearch', 'modules/'. $this->name .'/views/js/tmsearch.js');
        }
    }
    ...
    {
        if (!$this->isCached('tmsearch.tpl', $this->getCacheId())) {
           // $this->smarty->assign('content', $this->getWidgetVariables($hookName, $configuration));
        }

        return $this->display($this->_path, '/views/templates/hook/tmsearch.tpl', $this->getCacheId());
    }
}
