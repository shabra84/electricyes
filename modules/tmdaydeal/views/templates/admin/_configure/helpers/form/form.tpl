...
{block name="field"}
  {if $input.type == 'block_specific'}
    ...
    {addJsDefL name=tmdd_msg_included}{l s='tax included' mod='tmdaydeal'}{/addJsDefL}
    {addJsDefL name=tmdd_msg_excluded}{l s='tax excluded' mod='tmdaydeal'}{/addJsDefL}
    <div class="daydeal-alert-container">
      {foreach from=$specific_prices_data item=specific_prices name=specific_prices}
        <div class="daydeal-prices alert alert-warning">
          ...
          <p>{l s='Sale:' mod='tmdaydeal'}&nbsp;{$specific_prices['reduction']|escape:'htmlall':'UTF-8'}&nbsp;{$specific_prices['reduction_type']|escape:'htmlall':'UTF-8'}
          {if $specific_prices['reduction_type'] == 'amount'},&nbsp;
            {if $specific_prices['reduction_tax'] == 1}
              {l s='tax included' mod='tmdaydeal'}
            {else}
              {l s='tax excluded' mod='tmdaydeal'}
            {/if}
          {/if}
          </p>
