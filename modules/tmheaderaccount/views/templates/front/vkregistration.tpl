...
{block name='breadcrumb'}
  <nav class="breadcrumb hidden-sm-down">
    <ol itemscope itemtype="http://schema.org/BreadcrumbList">
      ...
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
          <span>
            <span itemprop="name">{l s='VK registration' mod='tmheaderaccount'}</span>
          </span>
      </li>
    </ol>
  </nav>
{/block}
{block name="content"}
<form action="{$link->getModuleLink('tmheaderaccount', 'vkregistration', array(), true)}" method="post" class="box">
  <div class="row">
    ...
    <div class="col-lg-6">
      ...
      <div class="form-group">
        ...
        <label class="radio-inline">
          <input type="radio" value="2" name="gender" {if $gender == 'male' || $gender == 2}checked{/if} />{l s='Male' mod='tmheaderaccount'}
        </label>
        <label class="radio-inline">
          <input type="radio" value="1" name="gender" {if $gender == 'famale' || $gender == 1}checked{/if} />{l s='Famale' mod='tmheaderaccount'}
        </label>
      </div>
      <div class="form-group">
