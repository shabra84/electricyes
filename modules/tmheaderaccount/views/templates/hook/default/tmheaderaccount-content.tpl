...
<ul class="header-login-content toogle_content{if $customer.is_logged} is-logged{/if}">
  ...
  <li {if $hook == 'displayNav2'}class="{$configs.TMHEADERACCOUNT_DISPLAY_STYLE|escape:'quotes':'UTF-8'}"{/if}>
    <ul>
      <li class="user-data">
        {if $configs.TMHEADERACCOUNT_USE_AVATAR}
          <img src="{$avatar}" alt=""{if $hook == 'displayNav'} width="{if $configs.TMHEADERACCOUNT_DISPLAY_STYLE == "twocolumns"}150{else}90{/if}"{/if}>
        {/if}
    ...
    <p class="logout">
      <a class="btn btn-default btn-md" href="{$link->getPageLink('index')|escape:'html':'UTF-8'}?mylogout" title="{l s='Sign out' mod='tmheaderaccount'}" rel="nofollow">
  ...
  <li class="create-account-content hidden">
    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" class="std">
      ...
      <div class="submit clearfix">
        ...
        <p>
          <a href="#" class="btn btn-primary btn-md signin"><span>{l s='Sign in' mod='tmheaderaccount'}</span></a>
        </p>
      </div>
    </form>
  </li>
