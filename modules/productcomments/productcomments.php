...
class ProductComments extends Module
{
	...
	public function uninstall($keep = true)
	{
		if (!parent::uninstall() || ($keep && !$this->deleteTables()) ||
			...
			!$this->unregisterHook('displayProductRating') ||
	...
	protected function _postProcess()
	{
		...
		$this->_clearcache('/views/templates/front/productcomments_reviews.tpl');
	}
	...
	public function getContent()
	{
		...
		//$this->context->controller->registerJavascript($this->name, 'modules/'.$this->name.'assets/js/moderate.js', ['position' => 'bottom', 'priority' => 150]);
	...
	public function displayApproveLink($token = null, $id, $name = null)
	{
		...
		//return $this->fetch('module:'.$this->name.'/views/templates/admin/list_action_approve.tpl');
	}
	...
	public function displayNoabuseLink($token = null, $id, $name = null)
	{
		...
		//return $this->fetch('module:'.$this->name.'/views/templates/admin/list_action_noabuse.tpl');
	}
	...
	public function hookDisplayProductTab($params)
	{
		...
		return $this->fetch('module:'.$this->name.'/views/templates/front/tab.tpl');
	...
	public function hookDisplayProductRating($params) {
		...
		require_once(dirname(__FILE__).'/ProductComment.php');
		$average = ProductComment::getAverageGrade((int)$params['product']);

		$this->context->smarty->assign(array(
			'averageTotal' => $average['grade'],
			'show_link' => true
		));
		return $this->fetch('module:'.$this->name.'/views/templates/front/productcomments_reviews.tpl');
	}

	public function hookDisplayProductListReviews($params)
	{
		$id_product = (int)$params['product']['id_product'];
		//if (!$this->isCached('/views/templates/front/productcomments_reviews.tpl', $this->getCacheId($id_product))) {
			require_once(dirname(__FILE__).'/ProductComment.php');
	...
	public function hookDisplayRightColumnProduct($params)
	{
		...
		return $this->fetch('module:'.$this->name.'/views/templates/front/productcomments-extra.tpl');
	}
	...
	public function hookDisplayComments($params)
  {
    require_once(dirname(__FILE__).'/ProductComment.php');
		...
		$id_guest = (!$id_customer = (int)$this->context->cookie->id_customer) ? (int)$this->context->cookie->id_guest : false;
		$customerComment = $pc->getByCustomer((int)(Tools::getValue('id_product')), (int)$this->context->cookie->id_customer, true, (int)$id_guest);

		$averages = $pc->getAveragesByProduct((int)Tools::getValue('id_product'), $this->context->language->id);
		$averageTotal = 0;
		...
		$data = array(
			...
			'action_url' => '',
			'product' => $product,
			'comments' => $pc->getByProduct((int)Tools::getValue('id_product'), 1, null, $this->context->cookie->id_customer),
			'criterions' => ProductCommentCriterion::getByProduct((int)Tools::getValue('id_product'), $this->context->language->id),
			'averages' => $averages,
			...
			'productcomment_cover_image' => $cover_image,
			'mediumSize' => '',//Image::getSize(ImageType::getFormatedName('medium')),
			'nbComments' => (int)$pc->getCommentNumber((int)Tools::getValue('id_product')),
		...
		return $this->fetch('module:'.$this->name.'/views/templates/front/productcomments.tpl');
  }

	public function hookDisplayProductTabContent($params, $extra = false)	{
...
//		print_r($params);
    ...
    //$get_values = $this->getProductCurrencyRow($id_product);
  	...
  	$this->context->controller->registerJavascript($this->name.'-rating', 'modules/'.$this->name.'/assets/js/jquery.rating.pack.js', ['position' => 'bottom', 'priority' => 150]);
		$this->context->controller->registerJavascript($this->name.'-textareaCounter', 'modules/'.$this->name.'/assets/js/jquery.textareaCounter.plugin.js', ['position' => 'bottom', 'priority' => 151]);
		$this->context->controller->registerJavascript($this->name, 'modules/'.$this->name.'/assets/js/productcomments.js', ['position' => 'bottom', 'priority' => 150]);

		$this->context->controller->registerStylesheet($this->name, 'modules/'.$this->name.'/assets/css/productcomments.css', ['media' => 'all', 'priority' => 150]);

		$this->page_name = Dispatcher::getInstance()->getController();
		if (in_array($this->page_name, array('product', 'productscomparison')))
		{
			//$this->context->controller->registerJavascript($this->name, 'modules/'.$this->name.'/assets/js/jquery.rating.pack.js', ['position' => 'bottom', 'priority' => 150]);
			if (in_array($this->page_name, array('productscomparison')))
			{
				$this->context->controller->addjqueryPlugin('cluetip');
				$this->context->controller->registerJavascript($this->name.'-comparison', 'modules/'.$this->name.'/assets/js/products-comparison.js', ['position' => 'bottom', 'priority' => 150]);
			}
		}
	}
	...
	public function hookExtraProductComparison($params)
	{
		...
		if (!empty($params['list_ids_product'])) {
			...
			return $this->fetch('module:'.$this->name.'/views/templates/front/products-comparison.tpl');
		}
	}
	...
	{
		return $this->fetch('module:'.$this->name.'/views/templates/front/icons.tpl');
	}
}
