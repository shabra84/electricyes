...
{if isset($menu) && $menu}
  <div class="{$hook}_menu top-level tmmegamenu_item default-menu top-global">
    <div class="menu-title tmmegamenu_item">{l s='Menu' mod='tmmegamenu'}</div>
    <ul class="menu clearfix top-level-menu tmmegamenu_item">
      {foreach from=$menu key=id item='item'}
        <li class="{$item.specific_class}{if $item.is_simple} simple{/if} top-level-menu-li tmmegamenu_item {$item.unique_code}">
          {if $item.url}
