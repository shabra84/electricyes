...
class Tmproductcustomtab extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->module_key = '1e9da70815a6163685f432e371003fbb';
    ...
    public function install()
    {
        ...
        && $this->registerHook('displayProductCustomTab')
        && $this->registerHook('displayProductCustomTabContent')
    ...
    protected function getConfigFormValues()
    {
        ...
        $fields_values = array(
            ...
            'selected_products' => array(
                ...
                'products' => $this->getProductsConfig(json_decode(Tools::getValue('selected_products', $tab->selected_products)))
            ),
        );
    ...
    protected function preValidateForm()
    {
        ...
        } elseif (!Validate::isGenericName(Tools::getValue('name_' . $this->default_language['id_lang']))) {
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        if (is_array($tabs)) {
            foreach ($tabs as $key => $tab) {
                $categories = json_decode($tab['selected_category']);
                if ($tab['selected_products']) {
                    if (in_array($id_product, json_decode($tab['selected_products']))) {
                        ...
                        $result[$tab['id_lang']][$key]['selected_products'] = implode(",", (array)json_decode($tab['selected_products']));
                        $result[$tab['id_lang']][$key]['iso_code'] = Language::getIsoById($tab['id_lang']);
                    }
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $this->context->controller->addJS($this->_path.'views/js/tmproductcustomtab_form.js');
    ...
    public function hookActionAdminControllerSetMedia()
    {
        if ($this->context->controller->controller_name == 'AdminProducts') {
            $this->context->controller->addJqueryUI('ui.sortable');
            $this->context->controller->addJS($this->_path.'/views/js/tmproductcustomtab_tab.js');
            $this->context->controller->addCSS($this->_path.'/views/css/tmproductcustomtab_admin.css');
            Media::addJsDefL('tmpst_theme_url', $this->context->link->getAdminLink('AdminTMProductCustomTab'));
        }
    }

    public function getShopTabs()
    {
    ...
    {
        $this->context->smarty->assign('items', $this->getShopTabs());
        return $this->display(__FILE__, 'views/templates/hook/tmproductcustomtab.tpl');
    }

    public function hookDisplayProductCustomTab()
    {
        $this->context->smarty->assign('items', $this->getShopTabs());
        return $this->display(__FILE__, 'views/templates/hook/tmproductcustomtab_nav.tpl');
    }

    public function hookDisplayProductCustomTabContent()
    {
        $this->context->smarty->assign('items', $this->getShopTabs());
        return $this->display(__FILE__, 'views/templates/hook/tmproductcustomtab_content.tpl');
    }
}
