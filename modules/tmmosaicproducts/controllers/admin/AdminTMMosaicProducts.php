...
class AdminTMMosaicProductsController extends ModuleAdminController
{
    ...
    public function ajaxProcessGetProductsById()
    {
        ...
        if (count($products_list) <= 0) {
            die(json_encode(array('response' => '<p class="no-products">'.$this->l('No products in this category.').'</p>')));
        }
        ...
        die(json_encode(array('response' => $this->products)));
    }
    ...
    public function ajaxProcessGetContent()
    {
        ...
        if (empty($this->html)) {
            die(json_encode(array('response' => '<p class="tmmp-no-item">'.$this->l('No item in this category.').'</p>')));
        }

        die(json_encode(array('response' => $this->html)));
    }
}
