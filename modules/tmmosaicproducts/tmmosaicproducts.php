...
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProducts.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsBanner.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsVideo.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsHtml.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsSlider.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsSliderSlide.php');
include_once(_PS_MODULE_DIR_.'tmmosaicproducts/src/MosaicProductsPresenter.php');

class Tmmosaicproducts extends Module implements WidgetInterface
{
    protected $html        = '';
    ...
    public function __construct()
    {
        ...
        $this->presenter = new MosaicProductsPresenter(
            Db::getInstance(),
            $this->context->shop,
            $this->name
        );
        $h = $this->presenter->getAllModuleHooks($this->name, $this->defaultHook);
        if (is_array($h)) {
            $this->hooks = $h;
        } elseif ($h) {
            $this->currentHook = $h;
    ...
    public function getContent(array $hooks = [])
    {
        ...
        if (Tools::getIsset('updatemosaicproducts') || Tools::getValue('updatemosaicproducts')) {
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProducts::getAssociatedIdsShop(
            ...
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProductsBanner::getAssociatedIdsShop(
                        (int)Tools::getValue('id_item')
                    )
                ) {
                    $output .= $this->getShopContextError($shop_context_id);
                } else {
            ...
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProductsVideo::getAssociatedIdsShop(
                        (int)Tools::getValue('id_video')
                    )
                ) {
                    $output .= $this->getShopContextError($shop_context_id);
                } else {
            ...
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProductsHtml::getAssociatedIdsShop(
                        (int)Tools::getValue('id_html')
                    )
                ) {
                    $output .= $this->getShopContextError($shop_context_id);
                } else {
            ...
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProductsSlider::getAssociatedIdsShop(
                        (int)Tools::getValue('id_slider')
                    )
                ) {
                    $output .= $this->getShopContextError($shop_context_id);
                } else {
            ...
            if ($this->checkContextShop()) {
                if ((int)Shop::getContextShopID() != $shop_context_id = MosaicProductsSliderSlide::getAssociatedIdsShop(
                        (int)Tools::getValue('id_slide')
                    )
                ) {
                    $output .= $this->getShopContextError($shop_context_id);
                } else {
    ...
    protected function renderForm()
    {
                ...
                'buttons' => array(
                    array(
                        'href'  => AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite(
                                'AdminModules'
                            ).($this->hooks ? '&hookName='.$this->currentHook : ''),
        ...
        if (Tools::getIsset('updatemosaicproducts') && (int)Tools::getValue('id_tab') > 0) {
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_tab', 'value' => (int)Tools::getValue(
                'id_tab'
            ));
        }
        $helper = new HelperForm();
        ...
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false)
            .'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.($this->hooks ? '&hookName='.$this->currentHook : '');
        $helper->token = Tools::getAdminTokenLite('AdminModules');
    ...
    {
        if (!$tabs = MosaicProducts::getTabList($this->currentHook)) {
            $tabs = array();
        }
        $fields_list = array(
            'name'               => array(
        ...
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name.($this->hooks ? '&hookName='.$this->currentHook : '');
    ...
    protected function updateTab()
    {
        ...
        $tab->hook_name = $this->currentHook;
        ...
        foreach ($this->languages as $lang) {
            if (!Tools::isEmpty(Tools::getValue('custom_name_'.$lang['id_lang']))) {
                $tab->custom_name[$lang['id_lang']] = pSql(Tools::getValue('custom_name_'.$lang['id_lang']));
            } else {
                $tab->custom_name[$lang['id_lang']] = pSql(
                    Tools::getValue('custom_name_'.$this->default_language['id_lang'])
                );
            }
        }
     ...
     * @return array $result
     */
    private function getTabListData($hookName)
    {
        $i = 0;
        $result = array();
        if (!$tab_list = MosaicProducts::getTabList($hookName, true)) {
            return false;
        }
    ...
    protected function updateBanner()
    {
        ...
        foreach ($this->languages as $lang) {
            if (!Tools::isEmpty(Tools::getValue('title'.$lang['id_lang']))) {
                $banner->title[$lang['id_lang']] = pSql(Tools::getValue('title_'.$lang['id_lang']));
            } else {
                $banner->title[$lang['id_lang']] = pSql(Tools::getValue('title_'.$this->default_language['id_lang']));
            }
            ...
            if (isset($_FILES['image_path_'.$lang['id_lang']])
                ...
                if ($error = ImageManager::validateUpload($_FILES['image_path_'.$lang['id_lang']])) {
                    $errors[] = $error;
                        ...
                        $_FILES['image_path_'.$lang['id_lang']]['tmp_name'], $temp_name
                    )
                ) {
                    return false;
                } elseif (!ImageManager::resize(
                    $temp_name, dirname(__FILE__).'/images/'.$salt.'_'.$_FILES['image_path_'.$lang['id_lang']]['name'],
                    null, null, $type
                )
                ) {
            ...
            'form' => array(
                ...
                'input'   => array(
                    ...
                    array(
                        ...
                        'name' => 'block_file',
                        'desc' => $this->l(
                            'Example adds video youtube and vimeo: https://www.youtube.com/embed/, https://player.vimeo.com/video/'
                        )
                    ),
                    array(
                        'type'   => 'switch',
    ...
    public function renderVideoList()
    {
        ...
        $helper->currentIndex = AdminController::$currentIndex
            .'&configure='.$this->name.'&id_shop='.(int)$this->context->shop->id.($this->hooks ? '&hookName='.$this->currentHook : '');
        $helper->tpl_vars = array(
    ...
    protected function addVideo()
    {
        ...
        $languages = Language::getLanguages(false);
        foreach ($languages as $language) {
            $video->title[$language['id_lang']] = pSql(Tools::getValue('title_'.$language['id_lang']));
            $video->url[$language['id_lang']] = pSql(Tools::getValue('url_'.$language['id_lang']));
    ...
    protected function checkVideoFields()
    {
        ...
        foreach ($this->languages as $lang) {
            if (!Tools::isEmpty(Tools::getValue('url_'.$lang['id_lang'])) && !$this->getVideoType(
                    Tools::getValue('url_'.$lang['id_lang'])
                )
            ) {
    ...
    protected function addHtml()
    {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $html->title[$lang['id_lang']] = pSql(Tools::getValue('title_'.$lang['id_lang']));
    ...
    protected function addSlider()
    {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $slider->title[$lang['id_lang']] = pSql(Tools::getValue('title_'.$lang['id_lang']));
        }
    ...
    protected function renderSlideForm($id_slide = false)
    {
        $fields_form = array(
            'form' => array(
                ...
                'buttons' => array(
                    array(
                        'href'  => AdminController::$currentIndex.'&configure='.$this->name.'&viewmosaicproductsslider&id_slider='.Tools::getValue(
                                'id_slider'
                            ).'&token='.Tools::getAdminTokenLite('AdminModules').($this->hooks ? '&hookName='.$this->currentHook : ''),
                        'title' => $this->l('Cancel'),
        ...
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name.'&viewmosaicproductsslide&id_slider='.Tools::getValue(
                'id_slider'
            ).($this->hooks ? '&hookName='.$this->currentHook : '');
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
    ...
    public function renderSliderSlideList()
    {
        ...
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name.'&id_slider='.Tools::getValue(
                'id_slider'
            ).($this->hooks ? '&hookName='.$this->currentHook : '');
        ...
        $helper->toolbar_btn['new'] = array(
            'href' => AdminController::$currentIndex.'&configure='.$this->name.'&addmosaicproductsslide&id_slider='.Tools::getValue(
                    'id_slider'
                ).'&token='.Tools::getAdminTokenLite('AdminModules').'&id_shop='.$this->id_shop.($this->hooks ? '&hookName='.$this->currentHook : ''),
            'desc' => $this->l('Add new')
        );
    ...
    private function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->l(
                'You cannot manage this module settings from "All Shops" or "Group Shop" context,
    ...
    protected function getShopContextError($id_shop = false)
    {
        if ($id_shop) {
            ...
            return $this->displayError(
                sprintf($this->l('You can only edit this element from the shop(s) context: %s'), $shop->name)
            );
        } else {
            return $this->displayError(
                sprintf($this->l('You cannot add/edit elements from a "All Shops" or a "Group Shop" context'))
            );
        }
    }
    ...
    protected function generateAdminItemData($data)
    {
        ...
        foreach ($get_rows as $row) {
            ...
            $row_content_explode = str_replace(
                ')', '', explode('(', str_replace('}', '', $row_data_explode[count($row_data_explode) - 1]))
            );
            if (count($row_content_explode) > 1) {
                    ...
                    if (!Tools::isEmpty($row_cont)) {
                        ...
                        if ($item_data_decode[1] && $check_data) {
                            ...
                            if ($this->getItemDataType($item_data_decode[1]) == 'prd') {
                                $product = new Product(
                                    $this->getItemDataId($item_data_decode[1]), true, $this->context->language->id
                                );
                                $link = new Link();
                                $id_image = $product->getCover($this->getItemDataId($item_data_decode[1]));
                                $image_path = $ssl.$link->getImageLink(
                                        $product->link_rewrite, $product->id.'-'.$id_image['id_image']
                                    );
                                $item_data[$item_data_decode[1]] = $image_path;
                            } elseif ($this->getItemDataType($item_data_decode[1]) == 'bnr') {
                                $banner = new MosaicProductsBanner(
                                    $this->getItemDataId($item_data_decode[1]), $this->context->language->id
                                );
                ...
                $output .= $this->display(
                    $this->local_path, 'views/templates/admin/layouts/'.str_replace('}', '', $row_type).'.tpl'
                );
            }
        }
        ...
        switch ($type) {
            case 'prd':
                $product = (new ProductAssembler($this->context))
                    ->assembleProduct(array('id_product' => $id));
    ...
    /**
     * Get path to frontend templates with overrides for the module
     * @return string
     */
    public function getPartialPath($hookName, $partial_name)
    {
        $hookPathName = Tools::strtolower($hookName);
        if (file_exists(_PS_THEME_DIR_.'modules/tmmosaicproducts/views/templates/hook/layouts/_partials/'.$hookPathName.'/'.$partial_name.'.tpl')) {
            $path = _PS_THEME_DIR_.'modules/tmmosaicproducts/views/templates/hook/layouts/_partials/'.$hookPathName.'/'.$partial_name.'.tpl';
        } else if (file_exists(
            _PS_THEME_DIR_.'modules/tmmosaicproducts/views/templates/hook/layouts/_partials/'.$partial_name.'.tpl'
        )) {
            $path = _PS_THEME_DIR_.'modules/tmmosaicproducts/views/templates/hook/layouts/_partials/'.$partial_name.'.tpl';
        } else if (file_exists(_PS_MODULE_DIR_.'tmmosaicproducts/views/templates/hook/layouts/_partials/'.$hookPathName.'/'.$partial_name.'.tpl')) {
            $path = _PS_MODULE_DIR_.'tmmosaicproducts/views/templates/hook/layouts/_partials/'.$hookPathName.'/'.$partial_name.'.tpl';
        } else {
    ...
    public function clearCache()
    {
        if ($this->hooks) {
            foreach ($this->hooks as $hook) {
                parent::_clearCache('tmmosaicproducts.tpl', $this->name.'_'.Tools::strtolower($hook['name']));
            }
        } else {
            parent::_clearCache('tmmosaicproducts.tpl', $this->name.'_'.Tools::strtolower($this->currentHook));
        }
    }
    ...
    {
        $this->context->controller->registerJavascript('module-tmmosaicproducts', 'modules/'.$this->name.'/views/js/tmmosaicproducts.js');
        $this->context->controller->registerStylesheet('module-tmmosaicproducts', 'modules/'.$this->name.'/views/css/tmmosaicproducts.css');
        $this->context->controller->registerJavascript('module-tmmosaicproducts-vid', 'modules/'.$this->name.'/views/js/video/video.js');
        $this->context->controller->registerStylesheet('module-tmmosaicproducts-vid', 'modules/'.$this->name.'/views/css/video/video-js.css');
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        $output = '';
        $result = array();
        $categories = $this->getTabListData($hookName);
        if ($categories) {
            foreach ($categories as $category) {
                ...
                foreach ($get_rows as $row) {
                        ...
                        foreach (array_filter($row_content_explode) as $row_cont) {
                            if (!Tools::isEmpty($row_cont)) {
                                ...
                                if ($item_data_decode[1] && $check_data) {
                                    // define all items type (bnr or prd it is)
                                    $elements_type[$item_data_decode[0]] = $this->getItemDataType($item_data_decode[1]);
                                    $elements_datas[$item_data_decode[0]] = $this->getFrontItemData(
                                        $item_data_decode[1]
                                    );
                                }
                            }
                    ...
                    $this->context->smarty->assign(
                        'partial_path', array(
                        'prd' => $this->getPartialPath($hookName, 'product'),
                        'bnr' => $this->getPartialPath($hookName, 'banner'),
                        'vd'  => $this->getPartialPath($hookName, 'video'),
                        'ht'  => $this->getPartialPath($hookName, 'html'),
                        'sl'  => $this->getPartialPath($hookName, 'slider')
                    )
                    );
                    $this->context->smarty->assign('row_content', $elements_data);
                    $this->context->smarty->assign('item_types', $elements_type);
                    ...
                    $output .= $this->display(
                        $this->local_path, 'views/templates/hook/layouts/'.str_replace('}', '', $row_type).'.tpl'
                    );
                }
    ...
    {
        $cacheName = 'tmmosaicproducts_'.Tools::strtolower($hookName);
        if (!$this->isCached('tmmosaicproducts.tpl', $this->getCacheId($cacheName))) {
            $this->smarty->assign('hookClass', Tools::strtolower($hookName));
            $this->smarty->assign('content', $this->getWidgetVariables($hookName, $configuration));
        }

        return $this->display($this->_path, '/views/templates/hook/tmmosaicproducts.tpl', $this->getCacheId($cacheName));
    }
}
