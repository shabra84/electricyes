...
class CategoryCore extends ObjectModel
{
    ...
    */
    ...
    public static function getCategories_home($idLang = false, $active = true, $order = true, $sqlFilter = '', $orderBy = '', $limit = '')
    {
        if (!Validate::isBool($active)) {
            die(Tools::displayError());
        }
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            '.Shop::addSqlAssociation('category', 'c').'
            LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON c.`id_category` = cl.`id_category`'.Shop::addSqlRestrictionOnLang('cl').'
            WHERE 1 '.$sqlFilter.' '.($idLang ? 'AND id_parent=\'2\' and `id_lang` = '.(int) $idLang : '').'
            '.($active ? 'AND `active` = 1' : '').'
            '.(!$idLang ? 'GROUP BY c.id_category' : '').'
