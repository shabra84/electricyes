...
class ProductCore extends ObjectModel
{
    ...
    */
    ...
    public static function getProducts_default(
        $id_lang,
        $start,
        $limit,
        $order_by,
        ...
        $sql = 'SELECT p.*, product_shop.*, pl.* , m.`name` AS manufacturer_name, s.`name` AS supplier_name
                ...
                LEFT JOIN `'._DB_PREFIX_.'supplier` s ON (s.`id_supplier` = p.`id_supplier`)'.
                ($id_category ? 'LEFT JOIN `'._DB_PREFIX_.'category_product` c ON (c.`id_product` = p.`id_product`)' : '').'
                WHERE p.id_category_default='.$id_category.' and pl.`id_lang` = '.(int)$id_lang.
                    ($id_category ? ' AND c.`id_category` = '.(int)$id_category : '').
                    ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').
                    ($only_active ? ' AND product_shop.`active` = 1' : '').'
                ORDER BY '.(isset($order_by_prefix) ? pSQL($order_by_prefix).'.' : '').'`'.pSQL($order_by).'` '.pSQL($order_way).
    ...
    public static function getFeatures_2($id_product, $id_lang)
    {
        if (!Feature::isFeatureActive()) {
            return array();
        }
        if (!array_key_exists($id_product, self::$_cacheFeatures)) {
            self::$_cacheFeatures[$id_product] = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                SELECT fp.id_feature, fp.id_product, fp.id_feature_value, fvl.value, fl.name
                FROM `'._DB_PREFIX_.'feature_product` fp
                ...
                WHERE `id_product` = '.(int)$id_product.' and fvl.id_lang='.$id_lang.' and fl.id_lang='.$id_lang
            );
        }
        return self::$_cacheFeatures[$id_product];
    }

    //$num_attribute=9 --> porque son 9 los atributos --> $tipo_armado,$tension_nominal,$frecuencia,$int_corto,$norma_d,$esquema_d,$tension_m,$borneos,$entrada_c
...
//        $sql = 'SELECT p.*, product_shop.*, pl.* , m.`name` AS manufacturer_name, s.`name` AS supplier_name
        $sql = 'SELECT p.id_product, pl.name , p.id_category_default, pc.id_product_attribute , count(pc.id_attribute) as num
                FROM `'._DB_PREFIX_.'product` p
                '.Shop::addSqlAssociation('product', 'p').'
                LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` '.Shop::addSqlRestrictionOnLang('pl').')
                LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa on pa.id_product = p.id_product
                LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pc on pa.id_product_attribute = pc.id_product_attribute
                WHERE p.id_category_default='.$id_category.' and pl.`id_lang` = '.(int)$id_lang.
                    ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').
                    ($only_active ? ' AND product_shop.`active` = 1' : ''). $where_attribute. '
                GROUP BY p.id_product, pc.id_product_attribute 
                HAVING num = '.$num_attribute.'
                ORDER BY '.(isset($order_by_prefix) ? pSQL($order_by_prefix).'.' : '').'`'.pSQL($order_by).'` '.pSQL($order_way).
                ($limit > 0 ? ' LIMIT '.(int)$start.','.(int)$limit : '');
                //echo $sql;
        $rq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
