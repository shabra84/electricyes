            ...
            {block name='facet_item_other'}
              <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if}">
                {foreach from=$facet.filters key=filter_key item="filter" name="filter"}
                  {if $filter.displayed}
                    <li class="facet_list{if $filter.active } active{/if}{if $smarty.foreach.filter.iteration > 5} unlimited{/if}">
                      <label class="facet-label{if $filter.active} active {/if}" for="facet_input_{$_expand_id}_{$filter_key}">
                        ...
                        {else}
                          <span class="custom-radio{if $facet.label == Price} hidden{/if}">
                            <input
                              id="facet_input_{$_expand_id}_{$filter_key}"
                              data-search-url="{$filter.nextEncodedFacetsURL}"
                              type="radio"
